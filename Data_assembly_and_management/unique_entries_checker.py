...
if st.session_state.original_df is not None:
            ...
            with col1:
                ...
                for i in range(0, len(unique_values), values_per_row):
                    ...
                    for j, val in enumerate(row_values):
                        with cols[j]:
                            count = df[df[selected_cat_col] == val].shape[0]
                            st.markdown(f"""
                            <div class="value-box">
            ...
            with col2:
                ...
                if st.button(f"Apply Changes to {selected_cat_col}", key=f"apply_cat_{selected_cat_col}"):
                    ...
                    st.session_state.edit_history.append({
                        ...
                        'type': 'categorical',
                        'action': f"Deleted {len(values_to_delete)} values, Applied replacements",
    ...
    if numeric_cols:
        ...
        problematic_values = find_non_numeric_in_numeric_cols(df, numeric_cols)
            ...
            if selected_num_col:
                ...
                with col1:
                    ...
                    for val in problematic_vals:
                        count = df[df[selected_num_col] == val].shape[0]
                        st.markdown(f"""
                ...
                with col2:
                    ...
                    if st.button(f"Apply Changes to {selected_num_col}", key=f"apply_num_{selected_num_col}"):
                        ...
                        st.session_state.edit_history.append({
                            ...
                            'type': 'numeric',
                            'action': f"Deleted {len(values_to_delete)} problematic values, Applied replacements",
                            'timestamp': datetime.now().strftime("%H:%M:%S")
                        })
